...
class DatasetProvider:
    def __init__(self, event_path: Path, gt_path: Path, mode: str = 'train', event_representation: str = 'voxel_grid',
        ...
        train_path = event_path / 'train'
        val_path = event_path / 'test'
        gt_train_path = gt_path / 'train'
        gt_val_path = gt_path / 'test'

        assert event_path.is_dir(), str(event_path)
        ...
        if mode == 'train':
            ...
            for child in train_path.iterdir():
                if any(k in str(child) for k in train_sequences_namelist):
                    train_sequences.append(Sequence(child, gt_train_path, 'train', event_representation, nr_events_data, delta_t_per_data,
            ...
            for child in val_path.iterdir():
                if any(k in str(child) for k in val_sequences_namelist):
                    val_sequences.append(Sequence(child, gt_val_path, 'val', event_representation, nr_events_data, delta_t_per_data,
                                                  nr_events_window, nr_bins_per_data, require_paired_data, normalize_event
                                                  , separate_pol, semseg_num_classes, augmentation, fixed_duration
                                                  , resize=resize))
                else:

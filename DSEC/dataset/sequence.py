...
class Sequence(Dataset):
    ...
    def __init__(self, seq_path: Path, gt_path: Path, mode: str='train', event_representation: str = 'voxel_grid',
        ...
        assert gt_path.is_dir()
        self.sequence_name = seq_path.name
        gt_path = gt_path / seq_path.name
        self.mode = mode
        ...
        # load timestamps
        self.timestamps = np.loadtxt(str(gt_path / (self.sequence_name + '_semantic_timestamps.txt')), dtype='int64')

        # load label paths
        if self.semseg_num_classes == 11:
            label_dir = gt_path / '11classes' 
        elif self.semseg_num_classes == 19:
            label_dir = gt_path / '19classes'
        else:
            raise ValueError
